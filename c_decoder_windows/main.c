...
S8 *word_registers[8] = {"ax", "cx", "dx", "bx", "sp", "bp", "si", "di"};
S8 *byte_registers[8] = {"al", "cl", "dl", "bl", "ah", "ch", "dh", "bh"};

S8 *instruction_name[256] = {
 [0] = "add",  
 [5] = "sub",  
 [7] = "cmp",  
};
...
USIZE common_displacement(S8 *name, U8 *bytes, USIZE pos);
USIZE common_immediate(U8 *bytes, USIZE pos);
USIZE mov_immediate_to_reg(U8 *bytes, USIZE pos);
USIZE immediate_accumulator(S8 *name, U8 *bytes, USIZE pos);
